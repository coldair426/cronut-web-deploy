...
import { ChevronDown, ChevronUp, Trash2, FoldVertical, UnfoldVertical, ChevronLeft } from 'lucide-react';
...
export default function OrderConfirmation({ params }: { params: { id: string } }) {
    ...
    const {
        ...
        queryKey: ['orderItemList', params.id],
        queryFn: async () => {
            try {
    ...
    const toggleAllAccordion = (open: boolean) => {
        setOpenIndices(open ? orderItemList.map((_: OrderItem, index: number) => index) : []);
    };
                ...
                <Box sx={{ width: '100%' }}>
                    <Box component="main" sx={{ mt: 2 }}>
                        ...
                        {orderItemList.length > 0 &&
                            orderItemList.map((item: OrderItem, index: number) => (
                                <Accordion
                                    key={index}
